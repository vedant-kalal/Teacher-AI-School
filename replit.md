...
12. **VideoCoordinatorAgent** - Plans educational videos showing processes, flows, and working models
...
3. **Video Planning** - VideoCoordinatorAgent analyzes which concepts benefit from motion visualization
4. **Pre-Generation** - Images and videos generated in parallel while lesson prepares
5. **Coordinated Display** - VisualCoordinatorAgent decides when to show each visual during narration
6. **Synchronized Delivery** - Visuals and videos appear at optimal moments while teacher explains
...
2. **Streaming Workflow Steps**
   ...
   6. Video display - Educational videos shown at relevant segments (non-blocking)
   7. Lesson completion - Summary and remaining visuals
...
4. **Frontend** (`web/src/`)
   ...
   - `components/StreamingBlackboard.tsx` - Real-time lesson display with:
     ...
     - **OpenAI Audio narration** - High-quality voice with ash/nova voices and caching
     - **AI-generated images** - Displayed at coordinated moments
     - **Educational videos** - Google Veo 3.1 generated videos showing processes and flows
...
- `video_ready` - Educational video ready to display (generated by Google Veo 3.1)
- `voice_audio` - OpenAI Audio realistic voice narration (Hinglish) with ash/nova voices
...
- Narration: Hinglish (Hindi-English mixed) like real Indian teachers, with OpenAI Audio voice (ash/nova voices)
- Educational videos: Google Veo 3.1 generated videos showing processes, flows, and working models with looping playback
- Voice: OpenAI Audio gpt-audio-mini model with ash (male) and nova (female) voices, includes audio caching
